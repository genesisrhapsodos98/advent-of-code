...
def _submit_answer(year, day, part, answer):
    ...
    if good_answer:
        ...
        if (day == 25 and part == 1) or (year >= 2025 and day == 12 and part == 1):
            # Immediately auto-submit day 25 part 2 for maximum efficiency
            _submit_answer(year, 25, 2, 1)

        if ((day != 25) or (year >= 2025 and day != 12)) or part != 2:
            if _account_selection is None:
...
def give_answer_current(part, answer):
    path = os.getcwd()
    segments = path.split('\\')
    day = int(segments[-1])
    year = int(segments[-2])
    give_answer(year, day, part, answer)
